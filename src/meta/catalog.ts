 ...
 * entry (placeholder colour or a real model). Pure functions over Profile.
 *
 * A "mount" is what the runner rides to shrug off one crash — a runic shield, a barrel, a ghost
 * horse. Mechanically it is one charge spent per ride (see game/hoverboard), cosmetically it is
 * the biggest personality slot in the game, which is why it has its own catalog.
 */
