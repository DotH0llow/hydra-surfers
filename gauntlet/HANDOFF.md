...
**Smaller gaps closed with this pass.** Who chases you is picked from the run seed out of five pursuers (guard, knight, inquisitor, king's collector, bounty hunter — `char.chaser.*`, dressed by manifest `meta`, so new ones are data). A broom joins the mounts. The profile shows top speed and Corrida do Dia runs. Two contracts added (face the weekly challenge, add up points across runs). The camera now kicks lightly on road events and perfect dodges (off with reduced motion). Runs carry the client `version` (`MIN_CLIENT` in the season content turns old builds away with 426) and the power-up held at the end, both for the balance queries in `docs/ONLINE.md`.
