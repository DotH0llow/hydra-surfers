...
Hydra Surfers ships as **one Cloudflare Worker with static assets**:
...
2. Project name: `hydra-surfers`. It must match `name` in `wrangler.jsonc`. If you choose another name, change `name` in `wrangler.jsonc` to match it.
3. Build settings:
...
6. `npm run assets:check`
7. `npx wrangler deploy --dry-run`
8. **Deploy** with `cloudflare/wrangler-action@v3` (`command: deploy`), on pushes to `main` only.
9. **Preview**: `wrangler versions upload` for pull requests opened from branches of this repository. The preview URL appears in the job log and in the Cloudflare dashboard under the Worker's **Deployments** tab.

Steps 8 and 9 run only when both secrets exist.
...
4. Push to `main`. The Worker named in `wrangler.jsonc` (`hydra-surfers`) is created on the first deploy and served at `https://hydra-surfers.<your-subdomain>.workers.dev`.
